...
knownImg = face_recognition.load_image_file("arslantest.jpg")

# Encode the known image
knownImg_encoding = face_recognition.face_encodings(knownImg)[0]
...
for image_path in Path("people").glob("*.jpg"):
    ...
    unknownImg = face_recognition.load_image_file(image_path)

    # Get the location of faces and face encodings for the current image
    face_encodings = face_recognition.face_encodings(unknownImg)

    # Get the face distance between the known person and all the faces in this image
    face_distance = face_recognition.face_distance(face_encodings, knownImg_encoding)[0]
